...
function printOutput (groups, writer, formatter) {
  for (const [level, group] of Object.entries(groups)) {
...
function serveOutput (title, output) {
  output = `
    <html>
      <head>
        <meta charset='utf-8'>
        <title>${title} - Gem progression</title>
      </head>
      <body style="background-color: #1E1F1C; color: #A38D6D; font-family: Verdana, Arial, Helvetica, sans-serif; font-size: 13px">
        <h1>${title}</h1>
...
function run () {
  ...
  if (args.serve) {
    ...
    let buffer = ''
    printOutput(groupedGroups, (str) => {
      buffer += str + format.newline
    }, format)
    serveOutput(data.name, buffer)
  } else {
    const format = {
      underline: (i) => `[u]${i}[/u]`,
      bold: (i) => `[b]${i}[/b]`,
    ...
    printOutput(groupedGroups, console.log, format)
  }
}
