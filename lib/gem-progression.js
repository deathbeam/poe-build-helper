...
function getGemData() {
  const questRewards = getData('quest-rewards')
  const vendorRewards = getData('vendor-rewards')
...
function createSkill(skill, gems) {
  ...
  gem['supports'] = (skill['supports'] || []).map(s => findGem(s, gems)).filter(e => !!e)
  return gem
}
...
function run(input) {
  const gems = getGemData()
  const data = loadYaml(input)
  const skills = data['skills'].map(s => createSkill(s, gems)).filter(e => !!e)
